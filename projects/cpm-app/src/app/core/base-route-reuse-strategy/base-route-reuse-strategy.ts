...
export class BaseRouteReuseStrategy implements RouteReuseStrategy {
  ...
  shouldAttach(route: ActivatedRouteSnapshot): boolean {
    const ret: boolean =
      !!route.routeConfig && !!this.cache[this.getUrl(route)];
    return ret;
  }
  ...
  */
  ...
  removeCacheItem(basePath: string, cascadeDelete: boolean): void {
    ...
    if (cascadeDelete === true) {
      const keys = _.filter(_.keys(this.cache), (k) => {
        return this.isSubPath(basePath, k);
      });
      _.forEach(keys, (k) => {
        delete this.cache[k];
      });
