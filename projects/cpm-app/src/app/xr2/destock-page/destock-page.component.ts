...
export class DestockPageComponent implements OnInit {
  ...
  screenState: DestockPageComponent.ListState =
    DestockPageComponent.ListState.NoData;
  ngUnsubscribe = new Subject();
  lastErrorMessage: string;
  eventDateTime: Date;

  searchFields = [nameof<IDestockTypeInfo>("Xr2DestockType_Display")];

  constructor(
    private destockService: DestockService,
    ...
    this.deviceId = Number.parseInt(
      activatedRoute.snapshot.paramMap.get("deviceId")
    );
    this.device$ = devicesService.getAllXr2Devices().pipe(
      shareReplay(1),
      map((devices: SelectableDeviceInfo[]) =>
        devices.find((d) => d.DeviceId === this.deviceId)
      )
    );
  }
  ...
  ngAfterViewInit(): void {
    this.destockEventConnectionService.DestockIncomingDataSubject.pipe(
      takeUntil(this.ngUnsubscribe)
    ).subscribe((event) => this.onDataReceived(event));
    this.destockEventConnectionService.DestockIncomingDataErrorSubject.pipe(
      takeUntil(this.ngUnsubscribe)
  ...
  setDestockService() {
    // Destock Service
    this.requestDeviceDestockTypeInfo$ = this.destockService
      .get(this.deviceId)
      .pipe(
        shareReplay(1),
        finalize(() => {
